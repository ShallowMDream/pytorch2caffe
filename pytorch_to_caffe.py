...
class TransLog(object):
    ...
    def init(self,inputs):
        ...
        """
        self.add_blobs(inputs,name='input')
...
def _conv2d(raw,input, weight, bias=None, stride=1, padding=0, dilation=1, groups=1):
    ...
    layer.conv_param(x.size()[1],weight.size()[2:],stride=_pair(stride),
                     pad=_pair(padding),bias_term=bias is not None,groups=groups)
    if bias is not None:
...
def trans_net(net,input_var,name='TransferedPytorchModel'):
    ...
    input_name = log.add_layer(name='input')
    layer = caffe_net.Layer_param(name=input_name, type='Input',top=["input1"])
    layer.input_param(input_var)
    log.cnet.add_layer(layer)
